...
var foundryAccountNames = [for config in aiServicesConfig: config.?resourceName ?? '${config.name}-${resourceSuffix}']
...
module modelDeployments 'deployments.bicep' = [for (config, i) in aiServicesConfig: {
  ...
  params: {
    ...
    modelsConfig: filter(modelsConfig, model => model.?aiservice == null || contains(foundryAccountNames[i], model.?aiservice))
  }
}]
